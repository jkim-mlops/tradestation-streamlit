...
def compute_bounds(row, resolution: str) -> tuple:
    entry = Timestamp(row["entry_time"]).tz_localize(None).to_pydatetime()
    exit_ = Timestamp(row["exit_time"]).tz_localize(None).to_pydatetime()
...
if __name__ == "__main__":
    ...
    entry_time = (
        Timestamp(trade["entry_time"]).tz_localize(None).to_pydatetime()
    )
    exit_time = (
        Timestamp(trade["exit_time"]).tz_localize(None).to_pydatetime()
    )
